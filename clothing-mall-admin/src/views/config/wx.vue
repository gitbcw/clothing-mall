      ...
      <el-tabs v-model="activeTab" tab-position="left">
        ...
        <el-tab-pane label="首页配置" name="home">
          ...
          <el-form-item label="启用活动位" prop="litemall_home_activity_enabled">
            <el-switch v-model="homeActivityEnabledBoolean" />
...
<script>
import { listWx, updateWx, listPromotion, updatePromotion } from '@/api/config'
...
export default {
  ...
  computed: {
    ...
    dataForm: {
      get() {
        return { ...this.wxData, ...this.promotionData }
      },
      set(val) {
        // 分离保存
      }
    ...
    birthdayCouponEnabled: {
      get() {
        return this.promotionData.litemall_birthday_coupon_status
      },
      set(val) {
        this.promotionData.litemall_birthday_coupon_status = val
      }
    },
    fullReductionStack: {
      get() {
        return this.promotionData.litemall_full_reduction_stack_with_coupon
      },
      set(val) {
        this.promotionData.litemall_full_reduction_stack_with_coupon = val
      }
    },
    homeActivityEnabledBoolean: {
      get() {
        return this.promotionData.litemall_home_activity_enabled === 'true'
      },
      set(val) {
        this.promotionData.litemall_home_activity_enabled = val ? 'true' : 'false'
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    parseActivityPages() {
      try {
        const json = this.promotionData.litemall_wework_activity_pages
        if (json && json.trim()) {
    ...
    serializeActivityPages() {
      if (this.activityPages.length > 0) {
        this.promotionData.litemall_wework_activity_pages = JSON.stringify(this.activityPages)
      } else {
        this.promotionData.litemall_wework_activity_pages = ''
      }
    },
    addActivityPage() {
    ...
    doUpdate() {
      ...
      // 分别保存两个配置
      const wxDataToSave = { ...this.wxData }
      const promotionDataToSave = { ...this.promotionData }

      Promise.all([
        updateWx(wxDataToSave),
        updatePromotion(promotionDataToSave)
      ])
        .then(() => {
          this.$notify.success({
            title: '成功',
        ...
        .catch(response => {
          this.$notify.error({
            title: '失败',
            message: response.data?.errmsg || '保存失败'
          })
        })
    }
  }
